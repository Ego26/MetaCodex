...
--
-- Verglichen wird ueber Knoten, nicht ueber Zauber-IDs. Der Grund steht
-- im Spiel: ein Spieler fuegte genau den angezeigten Build ein und las
-- danach "dir fehlt Hand von Gul'dan, du hast zusaetzlich Hand von
-- Gul'dan". Ein Talent hat mehrere IDs - die Definition eine, der
-- ersetzte Zauber eine zweite -, und die Quellen nennen mal die eine,
-- mal die andere. Der Baum kennt beide und weiss, dass sie an demselben
-- Knoten haengen.
do
    -- Ein Baum mit drei Knoten. Knoten 1 traegt zwei IDs fuer denselben
    -- Eintrag - genau der Fall, der oben gelogen hat.
    local mine = {
        nodes = {
            [1] = { entry = 11, rank = 1 },
            [2] = { entry = 21, rank = 2 },
        ...
        bySpell = {
            [100] = { node = 1, entry = 11 },   -- Definition
            [101] = { node = 1, entry = 11 },   -- ersetzter Zauber, gleicher Eintrag
            [200] = { node = 2, entry = 21 },
            [300] = { node = 3, entry = 31 },
            [310] = { node = 3, entry = 32 },   -- der andere Eintrag desselben Wahlknotens
            [400] = { node = 4, entry = 41 },
        },
    }

    local same = ns.Compare.Diff(mine, { nodes = {
        { spell = 100, rank = 1 }, { spell = 200, rank = 2 }, { spell = 300, rank = 1 },
    } })
    check("gleiche Wahl, kein Unterschied",
        same ~= nil and #same.missing == 0 and #same.extra == 0 and same.same == 3,
        same and (same.same .. " gleich, " .. #same.missing .. " fehlen, "
            .. #same.extra .. " zuviel") or "nichts")

    -- Dieselbe Wahl, nur unter der ANDEREN ID desselben Talents.
    local other = ns.Compare.Diff(mine, { nodes = {
        { spell = 101, rank = 1 }, { spell = 200, rank = 2 }, { spell = 300, rank = 1 },
    } })
    check("zweite ID desselben Talents ist kein Unterschied",
        other ~= nil and #other.missing == 0 and #other.extra == 0,
        other and (#other.missing .. " fehlen, " .. #other.extra .. " zuviel") or "nichts")

    local lacks = ns.Compare.Diff(mine, { nodes = {
        { spell = 100, rank = 1 }, { spell = 400, rank = 1 },
    } })
    check("fehlendes Talent wird genannt",
        lacks ~= nil and #lacks.missing == 1 and lacks.missing[1] == 400,
        lacks and table.concat(lacks.missing, ", ") or "nichts")

    local lower = ns.Compare.Diff(mine, { nodes = { { spell = 200, rank = 3 } } })
    ...
    -- Wahlknoten: derselbe Knoten, der andere Eintrag.
    local choice = ns.Compare.Diff(mine, { nodes = { { spell = 310, rank = 1 } } })
    check("anderer Eintrag am Wahlknoten ist ein Unterschied",
        choice ~= nil and #choice.missing == 1 and choice.missing[1] == 310,
        choice and table.concat(choice.missing, ", ") or "nichts")

    local extra = ns.Compare.Diff(mine, { nodes = { { spell = 100, rank = 1 } } })
    check("was du zusaetzlich hast, steht als Knoten da",
        extra ~= nil and #extra.extra == 2,
        extra and table.concat(extra.extra, ", ") or "nichts")

    -- Eine ID, die dieser Baum nicht kennt, wird uebergangen statt
    -- als Abweichung behauptet.
    local alien = ns.Compare.Diff(mine, { nodes = {
        { spell = 100, rank = 1 }, { spell = 200, rank = 2 },
        { spell = 300, rank = 1 }, { spell = 999999, rank = 1 },
    } })
    check("unbekannte ID wird uebergangen",
        alien ~= nil and #alien.missing == 0,
        alien and table.concat(alien.missing, ", ") or "nichts")

    check("ohne eigene Wahl kein Vergleich", ns.Compare.Diff(nil, { nodes = {} }) == nil)
    check("ohne Build kein Vergleich", ns.Compare.Diff(mine, nil) == nil)
end
