...
function Probe.Run()
    ...
    -- Der Talentvergleich, nachgerechnet.
    --
    -- Ob er stimmt, haengt daran, ob sich JEDE Zauber-ID des Builds auf
    -- einen Knoten dieses Baums bringen laesst. Der Baum kommt vom
    -- Client und gilt nur fuer die Klasse, auf der man steht - ausserhalb
    -- des Spiels ist das nicht zu pruefen. Hier steht die Zahl, und mit
    -- ihr laesst sich die Frage "passt das bei allen Klassen" Charakter
    -- fuer Charakter beantworten.
    do
        local mine = ns.Compare.Mine()
        local specID = ns.Compat.CurrentSpec()
        local _, build = ns.Recommend.Talents(specID, ns.Profile.Mode(), ns.Recommend.ALL)
        if not mine then
            line("  " .. L["PROBE_TALENTS_NONE"])
        elseif not build or not build.nodes then
            line("  " .. L["PROBE_TALENTS_NOBUILD"])
        else
            local known, unknown = 0, {}
            for _, node in ipairs(build.nodes) do
                if node.spell and mine.bySpell[node.spell] then
                    known = known + 1
                elseif node.spell and #unknown < 6 then
                    unknown[#unknown + 1] = tostring(node.spell)
                end
            end
            local picked = 0
            for _ in pairs(mine.nodes) do picked = picked + 1 end
            line("  " .. L["PROBE_TALENTS"], known, #build.nodes, picked)
            if #unknown > 0 then
                line("  " .. L["PROBE_TALENTS_UNKNOWN"], table.concat(unknown, ", "))
            end
            local diff = ns.Compare.Diff(mine, build)
            if diff then
                line("  " .. L["PROBE_TALENTS_DIFF"], #diff.missing, #diff.extra, diff.same)
            end
        end
    end
